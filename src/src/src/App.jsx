...
const LEVINE_STATES = [
  "CT","MA","ME","NH","NY","RI","VT","PA","NJ","MD","VA","WV","NC","SC","GA","FL","OH","MI","IN","KY","TN","AL","MS",
];
const MIDWEST_STATES = [
  "MN","WI","MI","MO","OH","IN","IL","IA","KS","NE","ND","SD","KY","TN","AL","MS","GA","FL","NC","SC",
];
...
const STATE_PRESETS = [
  { label: "All 50", states: ALL_STATES },
  { label: "Levine (23)", states: LEVINE_STATES },
  { label: "Non-Levine (27)", states: NON_LEVINE },
  { label: "Midwest+SE", states: MIDWEST_STATES },
  { label: "PA & NJ", states: ["PA", "NJ"] },
];
const MAX_LOG = 50;
...
function parseStates(s) {
  ...
  return s
    .split(",")
    .map((x) => x.trim())
    .filter((x) => ALL_STATES.includes(x));
}
...
function CapInput({ value, onChange, highlight, gradeColor }) {
  return (
    <input
      ...
      onChange={(e) =>
        onChange(
          e.target.value === "" ? 999 : Math.max(0, Math.min(999, parseInt(e.target.value) || 0))
        )
      }
      style={{
        width: 52,
        height: 30,
        border: `2px solid ${highlight ? gradeColor || "#6366f1" : "#e5e7eb"}`,
        borderRadius: 7,
        textAlign: "center",
        fontSize: 13,
        fontWeight: 700,
        color: value >= 999 ? "#94a3b8" : "#111827",
        fontFamily: "inherit",
        outline: "none",
        background: "white",
      }}
      onFocus={(e) => {
        e.target.style.borderColor = gradeColor || "#6366f1";
      }}
      onBlur={(e) => {
        e.target.style.borderColor = highlight ? gradeColor || "#6366f1" : "#e5e7eb";
      }}
    />
  );
}
      ...
      {items.map((s) => (
        <label
          ...
          style={{
            ...
            fontSize: 12,
            fontWeight: selected.includes(s) ? 700 : 400,
            color: selected.includes(s) ? "#6366f1" : "#374151",
            transition: "all .12s",
          }}
        >
          <input
      ...
      >
        <div
          style={{
            ...
            background: isInhouse ? "#fff5f5" : "#f8fafc",
            flexShrink: 0,
          }}
        >
        ...
        <div style={{ padding: "20px 24px", flex: 1, display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#0f172a", marginBottom: 10 }}>📤 Send Mode — what to send to this client</div>
            <div style={{ display: "flex", gap: 8 }}>
              {SEND_MODES.map((m) => (
                <button
                  key={m.value}
                  onClick={() => upd("sendMode", m.value)}
                  style={{
                    flex: 1,
                    padding: "10px 8px",
                    borderRadius: 10,
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#0f172a", marginBottom: 10 }}>👤 Contact (who to call if something breaks)</div>
            <div style={{ display: "flex", gap: 8 }}>
              <input
          ...
          <div>
            ...
            <textarea
              placeholder="e.g. Contract renewed Jan 2026. Prefer Grade A leads in the morning..."
              value={client.notes || ""}
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 10 }}>
              {STATE_PRESETS.map((p) => {
                const active =
                  JSON.stringify([...client.selectedStates].sort()) === JSON.stringify([...p.states].sort());
                return (
                  <button
                    key={p.label}
                    onClick={() => upd("selectedStates", [...p.states])}
                    style={{
                      padding: "4px 10px",
                      borderRadius: 999,
                      border: "1.5px solid",
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 10 }}>
              <button
                onClick={() => upd("selectedDiseases", [...ALL_DISEASES])}
                style={{
                  padding: "4px 10px",
                  borderRadius: 999,
                  ...
                  color: client.selectedDiseases.length === ALL_DISEASES.length ? "white" : "#0891b2",
                }}
              >
                All
              </button>
              <button
                onClick={() => upd("selectedDiseases", [])}
                style={{
                  padding: "4px 10px",
                  borderRadius: 999,
                  border: "1.5px solid #fca5a5",
  ...
  return (
    <div
      style={{
        ...
        boxShadow: isDragging ? "0 8px 24px rgba(99,102,241,.15)" : "none",
        transition: "all .15s",
      }}
    >
      <div
      ...
      <button
        onClick={onOpen}
        style={{ width: 148, flexShrink: 0, textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "inherit" }}
      >
        <div
          style={{
            fontSize: 13,
            fontWeight: 700,
            color: isInhouse ? "#dc2626" : "#111827",
        ...
        {client.contactName && (
          <div style={{ fontSize: 8, color: "#64748b", marginTop: 2 }}>👤 {client.contactName}{client.contactPhone ? " · " + client.contactPhone : ""}</div>
        )}
        {client.notes && (
          <div style={{ fontSize: 8, color: "#94a3b8", marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 140 }}>📝 {client.notes}</div>
        )}
...
function DraggableList({ clients, onReorder, onUpdate, onOpen, fillDataMap }) {
  ...
  const onDragOver = (e, i) => {
    ...
    const u = [...clients];
    const [m] = u.splice(dragIndex.current, 1);
    u.splice(i, 0, m);
    dragIndex.current = i;
    onReorder(u);
  };
  ...
  return (
    <div>
      {clients.map((c, i) => {
        const fd = fillDataMap[c.id] || {};
        const cap = fd.cap;
        const sent = fd.sent ?? 0;
        ...
        return (
          <div key={c.id} draggable onDragStart={(e) => onDragStart(e, i)} onDragOver={(e) => onDragOver(e, i)} onDragEnd={onDragEnd}>
            <ClientRow
              ...
              fillPct={fillPct}
              onChange={(upd) => onUpdate(c.id, upd)}
              onOpen={() => onOpen(c.id)}
            />
          </div>
        );
      ...
      {isCustomOrder ? (
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", background: "#fef9c3", border: "1.5px solid #fde047", borderRadius: 7 }}>
            <span>✏️</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: "#a16207" }}>Custom priority active</span>
          </div>
...
function ColHeaders() {
  const th = (l, w, c = false) => (
    <div style={{ width: w, fontSize: 8, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".07em", textAlign: c ? "center" : "left", flexShrink: 0 }}>
      {l}
    </div>
  );
  return (
...
function SummaryTab({ clients }) {
  ...
  const fillData = {};
  clients.forEach((c) => {
    fillData[c.id] = { sent: 0, cap: c.dailyCap || (c.gradeACap < 999 ? c.gradeACap + c.gradeBCap + c.gradeCCap : null) };
  });
  days.forEach((d) => {
    const dd = generateDemoData(clients, d);
    clients.forEach((c) => {
      fillData[c.id].sent += dd[c.id].sent;
    });
  });
  ...
  const rows = clients
    .filter(
      (c) =>
        filterType === "all" ||
        (filterType === "paying" && c.type === "Paying") ||
        (filterType === "inhouse" && c.type === "Inhouse") ||
        (filterType === "active" && c.on) ||
        (filterType === "paused" && !c.on)
    )
    .map((c) => {
      const d = fillData[c.id];
      const cap = d.cap ? d.cap * days.length : null;
  ...
  return (
    <div>
      <div style={{ padding: "10px 14px", background: "#eff6ff", border: "1.5px solid #bfdbfe", borderRadius: 10, marginBottom: 14, fontSize: 11, color: "#1e40af" }}>
        <strong>🔧 For your engineer:</strong> Replace <code>generateDemoData()</code> with a fetch to your Google Sheet or API. Required shape: <code>{"{ [clientId]: { sent: number, cap: number|null } }"}</code>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", background: "white", border: "1.5px solid #e2e8f0", borderRadius: 9 }}>
          ...
          <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} max={toDate} style={{ border: "none", fontSize: 12, fontWeight: 600, color: "#0f172a", fontFamily: "inherit", outline: "none", cursor: "pointer" }} />
          <span style={{ fontSize: 11, fontWeight: 600, color: "#64748b" }}>To</span>
          <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} min={fromDate} max={today()} style={{ border: "none", fontSize: 12, fontWeight: 600, color: "#0f172a", fontFamily: "inherit", outline: "none", cursor: "pointer" }} />
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[
            ...
            <button key={k} onClick={() => setFilterType(k)} style={{ padding: "4px 10px", borderRadius: 999, border: "1.5px solid", fontSize: 11, fontWeight: filterType === k ? 700 : 500, cursor: "pointer", fontFamily: "inherit", background: filterType === k ? "#111827" : "white", borderColor: filterType === k ? "#111827" : "#e2e8f0", color: filterType === k ? "white" : "#475569" }}>{v}</button>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginLeft: "auto" }}>
          ...
          {[
            ...
            <button key={k} onClick={() => setSortBy(k)} style={{ padding: "3px 9px", borderRadius: 6, border: "1.5px solid", fontSize: 11, fontWeight: sortBy === k ? 700 : 500, cursor: "pointer", fontFamily: "inherit", background: sortBy === k ? "#6366f1" : "white", borderColor: sortBy === k ? "#6366f1" : "#e2e8f0", color: sortBy === k ? "white" : "#475569" }}>{v}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "white", borderRadius: 12, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
        ...
        {rows.map((row, i) => {
          const b = badge(row);
          const mode = SEND_MODES.find((m) => m.value === row.sendMode) || SEND_MODES[0];
          return (
            <div
              key={row.id}
              style={{
                display: "grid",
                gridTemplateColumns: "2fr 1fr 1fr 1fr 3fr 1fr",
                padding: "11px 16px",
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 1 }}>
                  {row.type} · <span style={{ fontWeight: 600, color: mode.color }}>{mode.icon} {mode.label}</span>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center" }}>
                <span style={{ fontSize: 20, fontWeight: 900, color: row.sent === 0 && row.on ? "#d97706" : "#0f172a" }}>{row.sent}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center" }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: "#64748b" }}>{row.cap ?? "∞"}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center" }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: row.remaining === 0 ? "#dc2626" : row.remaining != null && row.remaining <= 5 ? "#d97706" : "#15803d" }}>{row.remaining ?? "∞"}</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 3, paddingRight: 12 }}>
                {row.pct != null ? (
                  <>
                    <span style={{ fontSize: 11, fontWeight: 700, color: row.pct >= 100 ? "#dc2626" : row.pct >= 80 ? "#d97706" : "#0f172a" }}>{row.pct}%</span>
                    <FillBar pct={row.pct} />
                  </>
                ) : (
                  <span style={{ fontSize: 10, color: "#94a3b8" }}>No cap</span>
                )}
              </div>
              <div style={{ display: "flex", alignItems: "center" }}>
                <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 9px", borderRadius: 999, background: b.bg, color: b.c }}>{b.l}</span>
              </div>
            </div>
          );
        })}
  ...
  if (log.length === 0) {
    return (
      <div style={{ textAlign: "center", padding: "60px 0", color: "#94a3b8", fontSize: 14 }}>
        No changes recorded yet. Changes will appear here as you edit clients.
      </div>
    );
  }
  return (
...
export default function App() {
  ...
  const resetGroupOrder = (gk) => {
    ...
    setClients((prev) => {
      const inGroup = prev.filter(gfn);
      const outGroup = prev.filter((c) => !gfn(c));
  ...
  const tabs = [
    ...
    ["summary", "📊 Fill Rate"],
    ["log", "📋 Log" + (changeLog.length > 0 ? " (" + changeLog.length + ")" : "")],
  ];
      ...
      <div style={{ background: "white", borderBottom: "1.5px solid #e2e8f0", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1140, margin: "0 auto", padding: "0 16px", display: "flex", alignItems: "center", gap: 10, height: 52 }}>
          ...
          <div style={{ display: "flex", gap: 1, background: "#f1f5f9", borderRadius: 8, padding: 3, flexShrink: 0 }}>
            {tabs.map(([k, v]) => (
              <button key={k} onClick={() => setActiveTab(k)} style={{ padding: "4px 12px", borderRadius: 6, border: "none", fontSize: 11, fontWeight: activeTab === k ? 700 : 500, cursor: "pointer", fontFamily: "inherit", background: activeTab === k ? "white" : "transparent", color: activeTab === k ? "#0f172a" : "#64748b", boxShadow: activeTab === k ? "0 1px 4px rgba(0,0,0,.08)" : "none", transition: "all .15s", whiteSpace: "nowrap" }}>{v}</button>
            ))}
          </div>
          ...
          {activeTab === "distribution" && (
            <>
              <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                {filterBtns.map(([k, v]) => (
                  <button key={k} onClick={() => setFilter(k)} style={{ padding: "3px 9px", borderRadius: 999, border: "1.5px solid", fontSize: 10, fontWeight: filter === k ? 700 : 500, cursor: "pointer", fontFamily: "inherit", background: filter === k ? "#111827" : "white", borderColor: filter === k ? "#111827" : "#e2e8f0", color: filter === k ? "white" : "#475569" }}>{v}</button>
                ))}
              </div>
              <input placeholder="🔍 Search…" value={search} onChange={(e) => setSearch(e.target.value)} style={{ height: 30, padding: "0 9px", border: "1.5px solid #e2e8f0", borderRadius: 7, fontSize: 11, fontFamily: "inherit", outline: "none", width: 120, background: "#f8fafc" }} onFocus={(e) => { e.target.style.borderColor = "#6366f1"; }} onBlur={(e) => { e.target.style.borderColor = "#e2e8f0"; }} />
            </>
          )}

          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setShowPauseConfirm(true)} style={{ padding: "5px 12px", borderRadius: 8, border: "1.5px solid #fca5a5", background: "#fff5f5", color: "#dc2626", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 4 }}>
              🚨 Pause All
            </button>
            {!saved ? (
              <>
              ...
              <span style={{ fontSize: 10, color: "#94a3b8" }}>✓ Saved</span>
            )}
          </div>
        </div>
      </div>
